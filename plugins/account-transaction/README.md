...
| `assetrelated-fix` | Any custody | Batch fixer for **income receipts** with missing `AssetRelated`. Targets `GENERAL LEDGER RECEIPT` rows whose `GeneralLedgerType = 'INTEREST/DIVIDEND'` (coupons, dividends, rendimentos, fee rebates) where `AssetRelated` is NULL — these don't tie back to the originating asset, so they vanish from return attribution. Parses the security out of the custody description (XP layouts), confirms it against assets the account actually holds/traded, and autonomously sets `AssetRelated` + promotes the row to `UPDATED` for high-conviction lock-aware matches. Handles **both** VALIDATED-with-NULL (scenario 1) and PENDING-income-stuck (scenario 2). Anything it can't prove is reported for human review. |
| `position-quantity-adjustment` | Any custody | Sweep absorber (not a loader) for the **fractional dust** that survives the daily `CustodyPosition` ↔ `AccountPosition` reconciliation — sub-1-unit quota drift on funds (e.g. Δ `+0.0647` on 524k of `Sten Master D30`) AND sub-currency-unit cash rounding gaps (e.g. Δ `+0.05` BRL). Given an account list and a date/window, joins both views per `(Account, Asset, Date)`, classifies each break into *dust* (auto-absorb) / *real* (report) / *missing* (report) under conservative kind-aware tolerances (`ASSET`: `ABS(Diff) < 1` *unit* AND `ABS(Diff)*Price < 5` in asset currency; `CASH`: `ABS(Diff) < 5` *currency units*), and dispatches by kind: non-cash assets → `BUY`/`SELL` for `ABS(Diff)` with `ValueGross=ABS(Diff)*<AssetData.v_Price>` and `Value=0` (no cash leg); cash (`Asset = Currency`) → `GENERAL LEDGER RECEIPT`/`DELIVERY` for `ABS(Diff)` with full cash impact (`Value=Quantity=ValueGross=ABS(Diff)`, `Price=1`, `AssetRelated=NULL`). Both shapes land `Status=UPDATED`, `Obs='automatic (quantity|cash) adjustment'`, `AgentCheck` tagged `[PQA]`. Lock-aware — **must run before the daily `CheckedDate` is advanced over the target date**; never moves the lock. Anything integer-sized, round-number-cash, above tolerance, custody-missing, or unpriced is reported, never absorbed. |
