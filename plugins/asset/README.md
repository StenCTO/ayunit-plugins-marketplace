...
|---|---|---|
| `asset-lookup` | Read-only (`Global.Asset` + `v_AssetCustody`) | **Mandatory pre-check before `asset-register`.** Takes any custody-side identifier (ISIN, CUSIP, CNPJ, ANBIMA/Fund/Class code, numeric custody code like BTG's `assetCode`, free-text description, unknown ticker) and resolves it to the canonical `Asset` code via `Global.Asset` + per-custody `Portfolio.v_AssetCustody`. Never registers. Usually reveals that the "unknown" instrument is already in the book — the fix is a mapping, not a duplicate registration. |
...
| `assetcustody-fill` | `Portfolio.AssetCustody` `I` + bulk `CustodyPosition` back-fill | Durable upstream fix for the recurring pattern where a custody loader receives a ticker/CNPJ/AnbimaCode that resolves to an already-registered `Global.Asset` but no per-custody translation row exists (leaving `CustodyPosition` with `Asset=NULL` and stuck PENDING transactions). INSERTs the missing `AssetCustody` mapping ONCE, then runs `Portfolio.CustodyPosition_Update @CMD='Update_Missing_Asset'` to back-fill custody positions in bulk, then hands the unblocked PENDING pks to `pending-revalidate`. Fixes at the mapping level instead of per-pk. |
| `register-debenture` | enrichment → `asset-register` / `Global.Asset` `U` gap-fill | BR debenture path starting from an **Anbima code or ISIN**. Fetches the Anbima register snapshot via `get_debenture_register_data` (issuer, maturity, remuneration expression, ISIN, Lei 12.431 status), parses the remuneration (`IPCA + x%` / `DI + x%` / `% do DI` / prefixada) into `FloatAsset`/`CouponType`/`Spread` decimals, and resolves-first: a NEW debenture becomes a pre-filled payload handed to `asset-register`; an ALREADY-registered one gets a field-by-field audit and a **gap-fill `U`** completing missing columns only (e.g. NULL `Issuer`, resolved against `Global.Issuer` first) — mismatches reported, never overwritten. Never INSERTs on its own; never writes Anbima pricing fields (`pu`, VNA, taxas). |
