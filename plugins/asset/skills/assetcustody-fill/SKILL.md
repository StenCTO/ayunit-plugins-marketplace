...
description: "Use when unmapped custody-side identifiers (ticker, CNPJ, AnbimaCode, numeric assetCode) resolve to an already-registered Global.Asset but no Portfolio.AssetCustody translation row exists — symptoms: CustodyPosition rows with Asset=NULL AssetR=<code>, groups of PENDING AccountTransaction rows sharing a CustodyIdentifier, 'the loader is missing this ticker mapping', 'BTG has unmapped assetCode Y', 'add a v_AssetCustody row for Z', 'back-fill custody positions where Asset is null'. The durable upstream fix at the mapping level: INSERTs the AssetCustody row ONCE, back-fills CustodyPosition in bulk via Update_Missing_Asset, then hands the unblocked PENDING pks to pending-revalidate — instead of patching transaction rows per-pk. Never creates Global.Asset rows."
---
