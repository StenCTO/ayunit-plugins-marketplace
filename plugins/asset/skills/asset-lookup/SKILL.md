---
name: asset-lookup
description: "Use when the user has any custody-side identifier — ISIN, CUSIP, CNPJ, ANBIMA/Fund/Class code, a numeric custody code like BTG's assetCode, a custody free-text description, or an unknown ticker — and needs the canonical Asset code in Global.Asset / the per-custody mapping in Portfolio.v_AssetCustody. Read-only, NEVER registers; the mandatory pre-check before asset-register. Triggers: 'what asset is <CNPJ>?', 'resolve this ISIN/CUSIP', 'is <code> registered?', 'the loader failed to identify this asset — check the book', 'BTG sent assetCode <n>, find the Asset', or when another skill finds a PENDING row with Asset=NULL and must decide between mapping an existing Asset vs handing off to asset-register. PT/EN."
---
